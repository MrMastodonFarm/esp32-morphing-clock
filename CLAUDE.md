...
**Weather.** `getOpenMeteoData()` hits `http://api.open-meteo.com/v1/forecast` (plain HTTP, 5-day daily block, Fahrenheit, hardcoded `America/New_York` timezone in the URL), parses with ArduinoJson into `forecast5Days[5]` / `minTemp[]` / `maxTemp[]` / `sunriseToday` / `sunsetToday`. Sunrise/sunset come back as local-time ISO strings (`2026-08-07T06:15`) because the request pins the timezone, so `formatSunTime()` just lifts `HH:MM` from offset 11 and converts to 12-hour. The response is 764 bytes and parses to 2087 of the 4096-byte `DynamicJsonDocument` (measured on 64-bit, where ArduinoJson slots are larger than on the ESP32) — adding more daily fields is possible but check that number. `wmoWeatherCodeMapping()` collapses WMO codes into six internal icon ids (0 sun, 1 cloud, 2 showers, 3 rain, 4 storm, 5 snow). `getOpenMeteoData()` makes exactly **one** attempt and returns `bool`; retries belong to `loop()`. (It used to recurse behind a blocking `delay(5000)` once `failCount > 3` — a threshold one call could never reach, so a failed fetch was really just abandoned until the next hourly refresh.)

**Startup reliability (fixed 2026-08-21).** The forecast used to be blank after many cold boots: `setup()` fetched the instant `WL_CONNECTED` came up, before DHCP/DNS had settled, the fetch failed, and nothing tried again for an hour — and the status line printed `Weather recvd!` regardless. Now `setup()` waits up to `WIFI_IP_WAIT_MS` for a lease before the first fetch; `loop()` retries on a doubling backoff (`WEATHER_RETRY_MIN_SEC`..`WEATHER_RETRY_MAX_SEC`, 30s→5min) while `weatherFailed` and only falls back to the hourly cadence after a success; each attempt is bounded by `WEATHER_HTTP_TIMEOUT_MS`. Every successful fetch is also saved to NVS (`Preferences` namespace `weather`), and a boot whose first fetch fails draws that cached forecast — only if under `WEATHER_CACHE_MAX_AGE_HOURS` (12) old — with the status `Weather: cached` while retries continue. The simulator stubs `Preferences` as always-empty (`sim/shims/Preferences.h`), so the cache path has no sim coverage — verify it by power-cycling a clock with WiFi down.

**Stale forecast is visible, not silent.** `weatherStale()` is true when the data on the panel is older than `WEATHER_STALE_AFTER_SEC` (3h — three missed hourly refreshes) or cannot be dated, whether it came from the cache or from a live fetch that has since stopped succeeding; `weatherDataEpoch` carries the timestamp (it rides along in the NVS blob). Both `displayTodaysTempRange()` renderers then draw the `min/max °F` line in `WEATHER_STALE_COLOR` — the same red as a dead outdoor sensor, deliberately one convention. It recomputes on every `displayWeatherData()` repaint, i.e. after each fetch attempt, so the flip lags the threshold by at most one retry interval (≤5 min). Pinned by `sim/scenarios/weather_stale.scn` (`weather_stale true` backdates `weatherDataEpoch`).
