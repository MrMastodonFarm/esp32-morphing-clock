...
**Sunrise/sunset** (`displaySunTimes()`) flank the big weather icon — rise on its left, set on its right — in the only two gaps the layout has left. There is no slack: 13px is exactly one TomThumb `H:MM`, the clock's lower-right segment reaches x=52 on digits like 0/6/8, and `displayWeatherForecast()` clears from x=98. Those gaps exist only because the seconds digits are commented out; `digit1`/`digit0` would sit at x=59 and x=72. Verified against a 24-hour sweep and worst-case minute digits in the simulator.
